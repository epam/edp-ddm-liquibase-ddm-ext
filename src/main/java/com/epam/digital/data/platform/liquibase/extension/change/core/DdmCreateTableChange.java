...
public class DdmCreateTableChange extends CreateTableChange {
    ...
    private ValidationErrors validateColumnTypeForAutoGeneratedValues(List<DdmColumnConfig> generatedColumns) {
        ValidationErrors validationErrors = new ValidationErrors();

        for (DdmColumnConfig column : generatedColumns) {
            if(!column.getType().equalsIgnoreCase(DdmConstants.TYPE_TEXT)) {
                validationErrors.addError("Column '" + column.getName() + "' in table '" + 
                    getTableName() + "' must be of type TEXT because it stores auto-generated values, but is of type '" + column.getType() + "'");
            }
        }
        return validationErrors;
    }
    ...
    public SqlStatement[] generateStatements(Database database) {
        if (Boolean.TRUE.equals(getHistoryFlag())) {
            ...
            statements.addAll(statementsForColumnsWithAutoGeneratedValues(getTableName(), getColumnsWithAutoGeneratedValues()));

            statements.addAll(statementForBulkLoad());
            return statements.toArray(new SqlStatement[0]);
        } else {
            SqlStatement[] baseStatements = super.generateStatements(database);
            List<SqlStatement> statements = new ArrayList<>();
            
            statements.addAll(Arrays.asList(baseStatements));
            statements.addAll(statementsForColumnsWithAutoGeneratedValues(getTableName(), getColumnsWithAutoGeneratedValues()));
    ...
    protected List<SqlStatement> statementsForColumnsWithAutoGeneratedValues(String tableName, List<DdmColumnConfig> generatedColumns) {
        List<SqlStatement> result = new ArrayList<>();
        for (DdmColumnConfig column : generatedColumns) {
            result.add(new DdmCreateSequenceStatement(tableName, column.getName()));
            result.add(new RawSqlStatement(String.format(GRANT_USAGE_ON_SEQUENCE, tableName, column.getName(), "application_role")));
    ...
    protected List<DdmColumnConfig> getColumnsWithAutoGeneratedValues() {
        return getColumns().stream()
            .map(DdmColumnConfig.class::cast)
            .filter(c -> c.getAutoGenerate() != null)
            .collect(Collectors.toList());
    }

    protected List<SqlStatement> statementForBulkLoad() {
        List<SqlStatement> statements = new ArrayList<>();
        if (Boolean.TRUE.equals(getBulkLoad())) {
            statements.add(DdmUtils.insertMetadataSql(DdmConstants.ATTRIBUTE_BULK_LOAD, getTableName(),
                    DdmConstants.ATTRIBUTE_BULK_LOAD, Boolean.toString(true)));
        }
        return statements;
    }
}
