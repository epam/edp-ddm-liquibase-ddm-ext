...
import static liquibase.DdmParameters.isAll;
import static liquibase.DdmParameters.isEmpty;
import static liquibase.DdmParameters.isHistory;
import static liquibase.DdmParameters.isPrimary;

import java.lang.reflect.Modifier;
import java.util.Collection;
import java.util.Objects;
import liquibase.DdmConstants;
import liquibase.DdmHistoryTableColumn;
...
public class DdmCreateTableChange extends CreateTableChange {
    ...
    protected CreateTableStatement createStatement(Database database) {
        ...
        for (DdmHistoryTableColumn column : parameters.getHistoryTableColumns()) {
            if (isEmpty(column.getScope())
                || isAll(column.getScope())
                || (isPrimary(column.getScope()) && (!historyTable.get()))
                || (isHistory(column.getScope()) && (historyTable.get()))) {
                statement.addColumn(
                    column.getName(),
                    DataTypeFactory.getInstance().fromDescription(column.getType(), database),
                    !isEmpty(column.getDefaultValueComputed()) ? new DatabaseFunction(column.getDefaultValueComputed()) : null);
            }
